...
# Register custom constructors for Ansible-specific YAML tags.
# !unsafe tells Ansible to treat the value as a literal string (no Jinja2 processing).
# PyYAML's safe_load doesn't know about it, so we add a handler that just returns the value as-is.
def _unsafe_constructor(loader, node):
    """Handle Ansible's !unsafe tag by returning the plain string value."""
...
def convert_playbook(source_file, target_file, checkpoint_id):
    ...
    if not playbook_data or not isinstance(playbook_data, list) or len(playbook_data) == 0:
        print(f"❌ Invalid playbook structure")
        return False
    
    play = playbook_data[0]
    if not isinstance(play, dict) or 'tasks' not in play:
        print(f"❌ Playbook doesn't have expected structure")
        return False
    ...
    if not last_status_var:
        print(f"⚠️  Warning: Could not find last status variable, using status_1")
...
def test_playbook(playbook_file, checkpoint_id, test_host):
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"✅ Test passed for {playbook_file.name}")
            return True
        else:
            print(f"❌ Test failed for {playbook_file.name}")
            print(f"Return code: {result.returncode}")
            print(f"STDOUT:\n{result.stdout[:1000]}")
            print(f"STDERR:\n{result.stderr[:1000]}")
            return False
    except subprocess.TimeoutExpired:
        print(f"❌ Test timed out for {playbook_file.name}")
        return False
    except Exception as e:
        print(f"❌ Error testing {playbook_file.name}: {e}")
        return False
...
def fix_playbook_issues(playbook_file, checkpoint_id, test_host):
    ...
    print(f"\n🔧 Attempting to fix issues in: {playbook_file.name}")
    ...
    try:
        ...
        print(f"❌ Error fixing playbook: {e}")
        return False
...
def main():
    ...
    if args.file:
        ...
        if not playbook_files[0].exists():
            print(f"❌ File not found: {playbook_files[0]}")
            return 1
    else:
        playbook_files = list(source_dir.glob('cis_audit_*.yml'))
    
    if not playbook_files:
        print(f"❌ No playbook files found in {source_dir}")
        return 1
    
    print(f"📦 Found {len(playbook_files)} playbook(s) to convert")
    ...
    for source_file in sorted(playbook_files):
        ...
        if not checkpoint_id:
            print(f"⚠️  Warning: Could not extract checkpoint ID from {source_file.name}, skipping")
        ...
        if target_file.exists():
            ...
            if target_mtime >= source_mtime:
                print(f"⏭️  Skipping {source_file.name} (target is newer or same age as source)")
                success_count += 1
                skipped_count += 1
                continue
    ...
    print("=" * 80)
    print(f"✅ Successfully converted: {success_count}/{len(playbook_files)}")
    if skipped_count > 0:
