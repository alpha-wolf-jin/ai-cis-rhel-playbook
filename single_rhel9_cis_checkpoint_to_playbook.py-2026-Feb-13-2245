...
This script:
1. Queries the CIS RHEL 9 Benchmark using RAG to get checkpoint audit info
...
VECTOR_STORE_DIR = Path(__file__).parent / "CIS_RHEL9_DATA_DEEPSEEK"
...
def load_or_create_vector_store():
    ...
    print(f"Loading CIS RHEL 9 Benchmark PDF from {PDF_PATH}...")
...
def create_cis_search_tool(vector_store):
    ...
    def search_cis_benchmark(query: str) -> str:
        """Search the CIS RHEL 9 Benchmark document for security control information.
...
def get_checkpoint_info_with_agent(vector_store, checkpoint: str, verbose: bool = False) -> dict:
    ...
    system_prompt = """You are a CIS RHEL 9 security expert. Your task is to find and extract detailed information about a specific CIS checkpoint.
...
def generate_playbook_requirements_from_checkpoint(checkpoint_info: dict) -> dict:
    ...
    # Use audit_procedure if available, otherwise use raw_agent_response
    ...
    is_complete_script = (
        source_text and (
            source_text.strip().startswith('#!/usr/bin/env bash') or
            source_text.strip().startswith('#!/bin/bash') or
            ('#!/usr/bin/env bash' in source_text and 'l_output' in source_text and 'unset a_' in source_text) or
            ('Complete Audit Script' in source_text and '#!/usr/bin/env bash' in source_text)
        )
    )
    ...
    if is_complete_script:
        ...
        if script_start > 0:
            ...
            # Look for usage instructions patterns
            if 'IF' in usage_text.upper() or 'verify:' in usage_text.lower() or 'Run the following script' in usage_text:
                usage_instructions = usage_text[:300]  # Limit length
        
        # Extract the script content
        if script_start > 0:
            script_content = source_text[script_start:].strip()
        else:
            script_content = source_text.strip()
        
        # Create a simple requirement that uses the script
...
def process_checkpoint(vector_store, checkpoint: str, args):
    ...
    print("\n" + "="*100)
    print(f"STEP 1: Querying CIS RHEL 9 Benchmark using Agent RAG for: '{checkpoint}'")
    print("="*100)
